...
class FitCountProApp(tk.Tk):
    def __init__(self):
        ...
        self.current_exercise = self.exercises["Squats"] # Default exercise
    ...
    def on_exercise_change(self, event):
        """Called when a new exercise is selected from the dropdown."""
        ...
        # Reset counters when changing exercise mid-session
        if self.is_running:
    ...
    def log_current_set(self):
        if self.current_exercise.get_counter() > 0:
            self.session_data.append({
                "Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "Exercise": self.current_exercise.name, # Use the dynamic name
